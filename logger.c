...
// static void unsafe_logger(Simulation* sim);
static void safe_logger(Simulation* sim);
/**
...
static void invariantCheck(Simulation* sim)
{
   ...
   {
      fprintf(stderr, "INVARIANT ERROR: sim->osophers not defined!!\n");
      exit(EXIT_FAILURE);
   }
   if (sim->waiter == NULL)
   {
...
static void safe_logger(Simulation* sim)
{
   ...
   {
     perror("Error on entering monitor (Logger)");
   ...
   {
     perror("Error on exiting monitor (Logger)");
     pthread_exit((void *) EXIT_FAILURE);
   }
}
